...
DWORD cOffset::FindClientTable()
{
	...
	DWORD PatternAddress = FindPattern( OFF_CLIENT_PATTERN , sizeof( OFF_CLIENT_PATTERN ) , HwBase , HwEnd , 0 );
...
{
	int patternLength = native_strlen( mask );
	bool found = false;
...
DWORD cOffset::FindPattern( PCHAR pattern , DWORD size , DWORD start , DWORD end , DWORD offset )
{
	bool found = false;

	for ( DWORD i = start; i < end - size; i++ )
	{
		found = true;
